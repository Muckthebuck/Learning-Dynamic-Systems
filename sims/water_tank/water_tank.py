...
class WaterTank:
    def __init__(self, dt=0.01, history_limit=10, noise_std=0.0, plot_system=True, visual=True,
        ...
        # Visualization parameters (can be changed dynamically)
    ...
    def punch_hole(self, rate=0.05, y=None):
        tank_top = self.padding_top
        tank_bottom = self.vis_height - self.padding_bottom
        if y is None:
            y = int(tank_bottom - self.state * (tank_bottom - tank_top))
    ...
    def step(self, u, r=None):
        ...
        for hole in self.holes:
            hole['rate'] = min(hole['rate'] * 1.001, 0.3)
        ...
        leak = 0
        for hole in self.holes:
            hole_height_norm = (tank_bottom - hole['y']) / (tank_bottom - tank_top)
            if self.state > hole_height_norm:
                leak += hole['rate']

        delta = u - leak * self.state + noise
        self.state += delta * self.dt
        self.state = np.clip(self.state, 0.0, 1.0)
        self.inflow = max(0.0, u)

        self.history.append([self.state, u, r if r is not None else 0.0])
    ...
    def _update_visual(self):
        ...
        tank_top = self.padding_top
        tank_bottom = self.vis_height - self.padding_bottom
        water_top = int(tank_bottom - self.state * (tank_bottom - tank_top))
        ...
        for i, hole in enumerate(self.holes):
            ...
            if self.state > hole_height_norm and np.random.rand() < hole['rate']:
        ...
        # Friendly labels
        level_percent = int(self.state * 100)
...
def test_water_tank_controller_forever():
    tank = WaterTank(noise_std=0.0, plot_system=True, visual=True)  # noise off for stability
