...
class d_tfs:
    ...
    def __init__(self, A: Tuple[Union[List[float], cp.ndarray], Union[List[float], cp.ndarray]]):
        ...
        A (tuple): A tuple containing two lists or arrays, the numerator and denominator coefficients.
                   Coefficients are in descending powers of z^-1. For example, [1 0.5 0.25 0] corresponds to 1 + 0.5z^-1 + 0.25z^-2 + 0z^-3
                   The first element of the tuple is the numerator, the second element is the denominator.
        """
...
class SPS_indirect_model:
    """
    Indirect Sign Perturbed Sums (SPS) model class.
    """
    ...
    def open_loop_sps(self, G_0: 'd_tfs', H_0: 'd_tfs', Y_t: cp.ndarray, U_t: cp.ndarray, n_a: int, n_b: int) -> Tuple[bool, cp.ndarray]:
        ...
        try:
            ...
            # Using the last N data points ...
            N_hat_par = N_hat[-self.N:]
            U_t_par = U_t[-self.N-1:-1]

            # ... calculate perturbed prediction errors and the associated output trajectories
            ...
            S = cp.sum(cp.square(cp.matmul(R_root_inv, weighted_sum)), axis=(1, 2)) # Calculate ||S_i(theta)||^2

            # Calculate the ranking
            combined = cp.array(list(zip(self.pi_order, S)))
            order = cp.lexsort(combined.T)
            rank_R = cp.where(order == 0)[0][0] + 1 # find the rank of ||S_0(theta)||^2 amongst the ||S_i(theta)||^2
